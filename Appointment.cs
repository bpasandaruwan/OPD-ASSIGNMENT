...
namespace OPD_ASSIGNMENT
{
    public partial class Appointment_From : Form
    {
        ...
        private void Refress_Load()
        {
            using (SqlConnection con = new SqlConnection(App_Connection.GetConnectionString()))
            {
                ...
                string sql = @"SELECT a.id AS ID, 
                                    ...
                                    INNER JOIN patient p ON a.patient_id = p.patient_Id
                                    INNER JOIN speciality_area sa ON a.specialty_area = sa.id 
                                    INNER JOIN users u ON u.id = a.medical_id ORDER BY a.id DESC
                                    ";
        ...
        {
            if(Login_User_Details.Logged_UserRole_ID == 1)
            {
                //MessageBox.Show("Admin");

            }
            else if(Login_User_Details.Logged_UserRole_ID == 5)
            {
                int set_appointment_id = Convert.ToInt32(dt_appointment.SelectedRows[0].Cells[0].Value);
                //MessageBox.Show("Res");
                Appointment_Edit_Form Appointment_Edit = new Appointment_Edit_Form();
                Appointment_Edit.Edit_appointment_ID = set_appointment_id;
                Appointment_Edit.ShowDialog();
                Refress_Load();
            }
